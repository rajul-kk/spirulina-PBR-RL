    ...
    def forward(self, obs: torch.Tensor, m_t_minus_1: torch.Tensor = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if m_t_minus_1 is None:
            ...
            if m_t_minus_1.dim() == 2:
                # m_t_minus_1 shape: (obs_dim, order)
                # A_d shape:         (obs_dim, order, order)
                m_t_A = torch.einsum('dj,dqj->dq', m_t_minus_1, A_d)
                m_t_B = obs_sym * B_d
            else:
                # Target batched shape...
...
class TDMPC2Agent:
    def __init__(self, obs_dim: int, action_dim: int, device: str = "cpu"):
        ...
        # Privileged Distillation Teacher (training-only — not used at deployment)
        ...
        self.distil_step = 0  # tracks warm-up for distillation weight
    ...
    def plan(self, obs: np.ndarray, m_t: torch.Tensor = None, horizon: int = 24,
        ...
        with torch.no_grad():
            ...
            for _ in range(num_iters):
                ...
                if explore_mode:
                    # ── OU Noise Trajectory Injection ──
                    # Generate temporally correlated noise for smooth exploration sweeps
                    theta, dt, sigma = 0.15, 0.05, 0.2
                    noise = torch.zeros(num_samples, horizon, self.action_dim, device=self.device)
                    x = torch.zeros(num_samples, self.action_dim, device=self.device)
                    for t in range(horizon):
                        x = x + theta * (0 - x) * dt + sigma * np.sqrt(dt) * torch.randn_like(x)
                        noise[:, t, :] = x
                ...
                if explore_mode:
                    ...
                    terminal_val = q_mean + beta * q_var
                else:
                    # ── Standard Conservative MPPI ──
                    terminal_val = torch.min(v1, v2)
    ...
    def update(self, batch_obs, batch_mt, batch_actions, batch_rewards, batch_next_obs, batch_next_mt, batch_dones,
        ...
        # Auxiliary Target: Predict the LAST step OD from the latent
        true_od = next_obs[:, 0] 
        ...
        # Teacher encodes ground-truth hidden state → student latent must match (stop-grad teacher)
        loss_distil = torch.tensor(0.0, device=self.device)
        if batch_priv is not None:
            priv = torch.tensor(batch_priv, dtype=torch.float32).to(self.device)
            z_teacher = self.priv_encoder(priv).detach()  # teacher latent — no grad to student
            z_student  = h                                 # student latent from CNN encoder
            loss_distil = F.mse_loss(z_student, z_teacher)
            distil_weight = min(self.distil_step / 50_000, 1.0) * 0.1  # warm-up over 50K steps
...
class ReplayBuffer:
    """Stores (OBS_DIM) and continuous m_t tensors instead of stacked flat obs."""
    def __init__(self, capacity: int, obs_dim: int = OBS_DIM, order: int = 16,
                 action_dim: int = 4, priv_dim: int = PRIV_DIM):
        self.capacity = capacity
        self.obs       = np.zeros((capacity, obs_dim), dtype=np.float32)
        self.mt        = np.zeros((capacity, obs_dim, order), dtype=np.float32)
        self.actions   = np.zeros((capacity, action_dim), dtype=np.float32)
        self.rewards   = np.zeros(capacity, dtype=np.float32)
        self.next_obs  = np.zeros((capacity, obs_dim), dtype=np.float32)
        self.next_mt   = np.zeros((capacity, obs_dim, order), dtype=np.float32)
        self.dones     = np.zeros(capacity, dtype=np.float32)
        self.priv      = np.zeros((capacity, priv_dim), dtype=np.float32)  # privileged state
        self.idx  = 0
        self.size = 0

    def add(self, obs: np.ndarray, mt: np.ndarray, action: np.ndarray, reward: float,
             next_obs: np.ndarray, next_mt: np.ndarray, done: bool, priv: np.ndarray = None):
        if mt is not None:
            if isinstance(mt, torch.Tensor):
                 mt = mt.cpu().numpy()
            self.mt[self.idx] = mt
        if next_mt is not None:
            if isinstance(next_mt, torch.Tensor):
                 next_mt = next_mt.cpu().numpy()
            self.next_mt[self.idx] = next_mt
    ...
    def sample(self, batch_size: int):
        ...
        return (
            ...
            self.next_mt[idxs],
            self.dones[idxs],
            self.priv[idxs],   # privileged state batch
        )

def train_var_mpc(resume: bool = False):
    import sys
    import os
    ...
    ACTION_REPEAT_BY_DIFF = {0: 12, 1: 12, 2: 6}
    ...
    checkpoint_dir = "model_data/varmpc_checkpoints"
    state_path = "model_data/varmpc_training_state.pkl"

    agent  = TDMPC2Agent(OBS_DIM, ACTION_DIM, device=device)
    buffer = ReplayBuffer(25_000, OBS_DIM, ORDER, ACTION_DIM)  # LMU continuous storage
    ...
    # ─── Resume Logic ───
    if resume:
        ckpt_path, ckpt_step = find_latest_checkpoint(checkpoint_dir, "varmpc_", "_steps.pth")
        ...
        if ckpt_path and state is not None:
            ...
            if ckpt_step is not None and abs(state_step - ckpt_step) > 2000:
                ...
                print(f"  [CONTINUE] Found latest Var-MPC checkpoint: {os.path.basename(ckpt_path)}")
    ...
    obs_buf = ObservationBuffer(obs_dim=OBS_DIM, order=ORDER)
    ...
    if has_restored_replay:
        ...
        for _ in range(2000):
            ...
            with torch.no_grad():
                obs_tensor = torch.tensor(next_raw_obs, dtype=torch.float32).unsqueeze(0).to(device)
                _, next_mt = agent.compressor(obs_tensor, cur_mt.to(device) if cur_mt is not None else None)

            obs_buf.set_state(next_mt)
            priv_pre = raw_env.get_privileged_state() if hasattr(raw_env, 'get_privileged_state') else None
            buffer.add(cur_obs, cur_mt.detach().cpu().numpy() if cur_mt is not None else np.zeros((1, OBS_DIM, ORDER)), action, reward, next_raw_obs, next_mt.detach().cpu().numpy(), done, priv=priv_pre)
    ...
    while global_step < TOTAL_TRAINING_STEPS:
        ...
        phase_repeat = ACTION_REPEAT_BY_DIFF[train_diff]
        ...
        print(f"\n[Chunk] train_diff=D{train_diff} | mastery_diff=D{current_difficulty} | init={init_cells:,} | steps={chunk_steps:,} | repeat={phase_repeat}")
        pbar = tqdm(range(chunk_steps), desc=f"Adaptive D{train_diff}")

        for step in pbar:
            if step % phase_repeat == 0:
                # Curriculum-friendly exploration: only D0 uses variance-max branch.
                explore = (train_diff == 0)
                beta = 2.0 if explore else 0.0
                action = agent.plan(raw_obs, obs_buf.get_state(), horizon=PLANNING_HORIZON,
                                    num_samples=MPPI_SAMPLES, num_iters=3,
                                    explore_mode=explore, beta=beta)
                if not explore:
                    noise_scale = max(0.01, 0.15 * (1.0 - (float(global_step) / 600_000.0)))
                    action = action + np.random.normal(0, noise_scale, size=ACTION_DIM)
                action = np.clip(action, -1.0, 1.0)

                mean_od_str = f"{np.mean(recent_ods):.4f}" if recent_ods else "--"
                postfix_dict = {"Diff": f"D{train_diff}", "Mastery": f"D{current_difficulty}",
                                "Stir": f"{action[0]:.2f}", "Light": f"{action[1]:.2f}",
                                "Nutri": f"{action[2]:.2f}", "CO2": f"{action[3]:.2f}",
                                "Ep": episodes_this_chunk, "OD": mean_od_str}
                if metrics:
                    postfix_dict["W_Loss"] = f"{metrics['loss/consistency']:.3f}"
                    postfix_dict["R_Loss"] = f"{metrics['loss/reward']:.3f}"
            ...
            obs_buf.set_state(next_mt)
            priv_state = raw_env.get_privileged_state() if hasattr(raw_env, 'get_privileged_state') else None
            buffer.add(cur_obs, cur_mt.detach().cpu().numpy() if cur_mt is not None else np.zeros((1, OBS_DIM, ORDER)), action, reward, next_raw_obs, next_mt.detach().cpu().numpy(), done, priv=priv_state)
            ...
            if buffer.size >= BATCH_SIZE:
                ...
                metrics = agent.update(ob_b, mt_b, act_b, rew_b, nob_b, nmt_b, done_b, batch_priv=priv_b)

            if done:
                episodes_this_chunk += 1
                ep_pop = getattr(raw_env, 'num_active', 0)
                ep_od = episode_peak_od
                recent_rewards.append(float(episode_reward))
                recent_pops.append(float(ep_pop))
                recent_ods.append(float(ep_od))
                ...
                pbar.set_postfix({"Diff": f"D{train_diff}", "Mastery": f"D{current_difficulty}",
                                  "Ep": episodes_this_chunk, "OD": f"{mean_od:.4f}",
                                  "AvgRew": f"{mean_rew:.1f}", "PopAvg": f"{mean_pop:.0f}"})

            if global_step % 2000 == 0:
                os.makedirs(checkpoint_dir, exist_ok=True)
                agent.save(f"{checkpoint_dir}/varmpc_{global_step}_steps.pth")
                save_state(
                    state_path,
                    {
                        ...
                        "replay_buffer": replay_buffer_state(
                            buffer,
                            ["obs", "mt", "actions", "rewards", "next_obs", "next_mt", "dones", "priv"],
                        ),
                    },
                )
    ...
    agent.save("model_data/varmpc_genetic_ibm.pth")
    save_state(
        state_path,
        {
            "global_step": global_step,
...
def finetune_var_mpc(extra_steps: int = 500_000):
    ...
    model_path = "model_data/varmpc_genetic_ibm.pth"
    if not os.path.exists(model_path):
        print(f"  [ERROR] No saved model found at {model_path}")
        print("  Run 'python Var_MPC.py' first to complete the curriculum.")
        return
    ...
    buffer = ReplayBuffer(25_000, OBS_DIM, ORDER, ACTION_DIM)
    ...
    for _ in range(5000):
        ...
        buffer.add(cur_obs, cur_mt.detach().cpu().numpy() if cur_mt is not None else np.zeros((1, OBS_DIM, ORDER)), action, reward, next_raw_obs, next_mt.detach().cpu().numpy(), done)
        raw_obs = next_raw_obs
    ...
    for step in pbar:
        ...
        cur_hist = obs_buf.get()
        next_raw_obs, reward, terminated, truncated, _ = raw_env.step(action)
        done = terminated or truncated
        obs_buf.push(next_raw_obs)
        next_hist = obs_buf.get()
        buffer.add(cur_hist, action, reward, next_hist, done)
        episode_reward += reward
        ...
        if done:
            episodes += 1
            pbar.set_postfix({"Ep": episodes, "Pop": raw_env.num_active,
                               "Rew": f"{episode_reward:.1f}",
                               "Stir": f"{action[0]:.2f}", "Light": f"{action[1]:.2f}", "Nutri": f"{action[2]:.2f}"})
            raw_obs, _ = raw_env.reset()
            obs_buf.reset(raw_obs)
            episode_reward = 0.0

        if step % 20_000 == 0 and step > 0:
            os.makedirs("model_data/varmpc_checkpoints", exist_ok=True)
            agent.save(f"model_data/varmpc_checkpoints/varmpc_finetune_{step}_steps.pth")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Var-MPC for GeneticPBR")
    ...
    if args.finetune is not None:
        finetune_var_mpc(extra_steps=args.finetune)
    else:
        train_var_mpc(resume=args.resume)
